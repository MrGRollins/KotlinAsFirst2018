...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    for (wordTXT in substrings) {
        result[wordTXT] = wordTXT.toRegex(RegexOption.IGNORE_CASE)
                .findAll(txt).toList().size
    }
    return result
}
...
fun centerFile(inputName: String, outputName: String) {
    val readlines = File(inputName).readLines().map { it.trim() }

    val maxLines = readlines.map { it.length }.max() ?: 0

    File(outputName)
            .writeText(readlines.joinToString("\n") {
                " ".repeat((maxLines - it.length) / 2) + it
            })
...
fun top20Words(inputName: String): Map<String, Int> {
    val wordRegex = Regex("""([a-zA-Zа-яА-ЯёЁ]+)""", RegexOption.IGNORE_CASE)
