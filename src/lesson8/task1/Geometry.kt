...
data class Circle(val center: Point, val radius: Double) {
     ...
     */
    fun contains(p: Point): Boolean = sqr(radius) >= sqr(center.distance(p))
}
...
fun diameter(vararg points: Point): Segment {
    ...
    var maxSegment = Segment(points[0], points[2])

    for (i in 0 until points.size - 1)
        for (k in i + 1 until points.size) {
            val segmentPoint = Segment(points[k], points[i])

            if (segmentPoint.toString().length > maxSegment.toString().length)
...
fun circleByDiameter(diameter: Segment): Circle {
    val pointCenterX = (diameter.begin.x + diameter.end.x) / 2
    val pointCenterY = (diameter.begin.y + diameter.end.y) / 2
    val center = Point(pointCenterX, pointCenterY)
...
fun circleByThreePoints(a: Point, b: Point, c: Point): Circle {
    val sectorOne = bisectorByPoints(a, b)
    val sectorTwo = bisectorByPoints(b, c)
    val center = sectorOne.crossPoint(sectorTwo)
    val radius = max(
            max(center.distance(a), center.distance(b)), center.distance(c))
